...
export default function HomePage() {
  ...
  // 템플릿 프롬프트(버튼)와 사용자 입력 프롬프트 분리
  const templatePrompts = selectedPrompts.join(', ')
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-green-50 dark:bg-green-950/20 rounded-lg p-4 border border-green-300 dark:border-green-700">
                    ...
                    <div className="p-2 bg-white dark:bg-gray-900 rounded text-xs text-gray-700 dark:text-gray-300 border border-green-200 dark:border-green-800">
                      {(() => {
                        const parts = []
                        if (selectedTemplate) parts.push(selectedTemplate.prompt)
                        if (templatePrompts) parts.push(templatePrompts)
                        if (userCustomPrompt) parts.push(userCustomPrompt)
                        return parts.join(', ') || '프롬프트를 입력해주세요'
                      })()}
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-blue-50 dark:bg-blue-950/20 rounded-lg p-4 border-2 border-blue-300 dark:border-blue-700">
                    ...
                    {(templatePrompts || userCustomPrompt) ? (
                      <div className="p-3 bg-white dark:bg-gray-900 rounded border border-blue-200 dark:border-blue-800">
                        <p className="text-sm text-gray-700 dark:text-gray-300">
                          {userCustomPrompt || "사용자 입력 지시사항이 없습니다"}
                        </p>
                        {selectedPrompts.length > 0 && (
                          <div className="mt-2 pt-2 border-t border-blue-200 dark:border-blue-800">
  ...
  const canProceed = () => {
    ...
    if (mainMode === 'generate') {
      switch (currentStep) {
        ...
        case 2: return (templatePrompts.length > 0 || userCustomPrompt.length > 0) || selectedTemplate !== null
    ...
    if (mainMode === 'edit') {
      switch (currentStep) {
        ...
        case 4: return (templatePrompts.length > 0 || userCustomPrompt.length > 0) || selectedTemplate !== null
  ...
  const handleGenerate = async () => {
    ...
    try {
      // 프롬프트 조합: 템플릿 + 버튼 선택 + 사용자 입력
      const promptParts = []
      if (selectedTemplate) promptParts.push(selectedTemplate.prompt)
      if (templatePrompts) promptParts.push(templatePrompts)
      if (userCustomPrompt) promptParts.push(userCustomPrompt)
